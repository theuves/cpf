...
test('should return true for partially valid CPF in non-strict mode', t => {
  ...
  t.true(check(cpf1, { strict: false }))
  t.true(check(cpf2, { strict: false }))
  t.true(check(cpf3, { strict: false }))
})
...
test('should return false for CPF with invalid characters in non-strict mode', t => {
  ...
  t.false(check(cpf1, { strict: false }))
  t.false(check(cpf2, { strict: false }))
  t.false(check(cpf3, { strict: false }))
})
...
test('should return false if the input is not a string', t => {
  ...
  t.false(check(cpf3, { strict: false }))
  // @ts-expect-error invalid type
  t.false(check(cpf4, { strict: false }))
  // @ts-expect-error invalid type
  t.false(check(cpf5, { strict: false }))
})
